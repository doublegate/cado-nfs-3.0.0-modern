...
class FixedHTTPServer(http.server.HTTPServer):
    ...
    def handle_error(self, request, client_address):
        """ Suppress the noisy traceback printed when a client closes the
        connection before sending a request. The most common case is the SSL
        certificate-download probe in cado-nfs-client's get_ssl_certificate(),
        which completes the TLS handshake purely to read the server cert and
        then disconnects. Genuine handler errors are still reported. """
        exc = sys.exc_info()[1]
        if isinstance(exc, (BrokenPipeError, ConnectionResetError,
                            ConnectionAbortedError)):
            self.logger.debug("Client %s closed the connection before sending "
                              "a request (%s)", client_address, exc)
            return
...
if HAVE_SSL:
    class HTTPSServer(FixedHTTPServer):
        def __init__(self, server_address, HandlerClass, *args, certfile=None,
            ...
            # First init an SSL context with the key. PROTOCOL_SSLv23/
            # PROTOCOL_TLS are deprecated since Python 3.10; PROTOCOL_TLS_SERVER
            # is the correct server-side choice and negotiates the best
            # mutually-supported TLS version.
...
"""
    ...
    def create_certificate(self):
        ...
        # OpenSSL 3.x's default security level rejects RSA keys < 2048 bits
        # ("ee key too small"), so the original 1024-bit cert no longer works.
