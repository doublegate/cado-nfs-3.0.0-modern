...
void cpubinder::read_param_list(param_list_ptr pl, int want_conf_file)
{
    ...
    /* we must make sure that I/O objects are not loaded into the
     * topology, but it's likely that they're off by default anyway */
#if HWLOC_API_VERSION >= 0x00020000
    /* hwloc-2.0 removed the WHOLE_IO / IO_DEVICES / IO_BRIDGES topology
     * flags. I/O devices are now governed by a dedicated type filter, and
     * are excluded from the topology by default; make that explicit. Must
     * be called after hwloc_topology_init() and before _load(). */
