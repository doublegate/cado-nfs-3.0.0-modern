    ...
    def _urlopen_maybe_https(request, cafile=None, check_hostname=True):
        ...
        if scheme == "https":
            if sys.version_info[0] == 3:
                ...
                #
                # urlopen() dropped the cafile=/capath=/cadefault= parameters
                # in Python 3.12, so we build an SSLContext from the CA file
                # (the pinned, self-signed server certificate) and pass it via
                # context=. create_default_context() sets verify_mode to
                # CERT_REQUIRED and check_hostname to True; the latter can be
                # turned off when the user requested NO_CN_CHECK.
